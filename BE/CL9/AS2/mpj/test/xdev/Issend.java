...
public class Issend {
    ...
    public Issend(String args[]) throws Exception {
	/*
	 * args[0]=0
	...
	if( Integer.parseInt(args[0]) == 0 ) {
	   ...
	   sbuf.commit();
	   request = device.issend(sbuf, ids[1], 10, 10);
	   request.iwait();
	   sbuf.clear();
	   //Basic datatype case ..
	   ...
	   sbuf.putSectionHeader(Type.OBJECT);
           ...
           sbuf.commit();
           request = device.issend(sbuf, ids[1], 11, 11);
	   request.iwait();
	   sbuf.clear();
	   // Java object case 
	   ...
	   sbuf.putSectionHeader(Type.OBJECT);
           ...
           sbuf.commit();
           request = device.issend(sbuf, ids[1], 13, 13);
	   request.iwait();
	   sbuf.clear();  
	   // ANY_TAG case ..
	   ...
	   sbuf.putSectionHeader(Type.OBJECT);
           sbuf.write(source, 0, 1);
           sbuf.commit();
           request = device.issend(sbuf, ids[1], 12, 12);
	   request.iwait();
	   sbuf.clear();  
	   ...
	   sbuf.putSectionHeader(Type.OBJECT);
           sbuf.write(source, 0, 1);
           sbuf.commit();
           request = device.issend(sbuf, ids[1], 14, 14);
	   request.iwait();
	   sbuf.clear();  
