...
public class BufferTest3 {
  ...
  public BufferTest3(String args[]) throws Exception {
    ...
    if (size > 2) {
      if (id == 1)
	System.out.println("BufferTest3: must run with 2 processes");
      MPJDev.finish();
      return;
    }
	...
	byteArray[i] = 's';
	  ...
	  BufferFactory.create(80 + 8 + SEND_OVERHEAD), SEND_OVERHEAD,
	  80 + 8 + SEND_OVERHEAD);
	  ...
	  BufferFactory.create(160 + 8 + SEND_OVERHEAD), SEND_OVERHEAD,
	  160 + 8 + SEND_OVERHEAD);
	  ...
	  BufferFactory.create(320 + 8 + SEND_OVERHEAD), SEND_OVERHEAD,
	  320 + 8 + SEND_OVERHEAD);
	...
	byteReadArray[i] = 'x';
      ...
      Buffer intBuffer = new Buffer(
	  BufferFactory.create(160 + 8 + RECV_OVERHEAD), RECV_OVERHEAD,
	  160 + 8 + RECV_OVERHEAD);
      ...
      Buffer doubleBuffer = new Buffer(
	  BufferFactory.create(320 + 8 + RECV_OVERHEAD), RECV_OVERHEAD,
	  320 + 8 + RECV_OVERHEAD);
      ...
      Buffer charBuffer = new Buffer(
	  BufferFactory.create(80 + 8 + RECV_OVERHEAD), RECV_OVERHEAD,
	  80 + 8 + RECV_OVERHEAD);
    ...
    if (id == 1)
      System.out.println("BufferTest3 TEST Completed (?)" + id);
